...
class SegPatchDataset(Dataset):
    ...
    def __init__(
        ...
        pairs: Optional[List[Tuple[str, str]]] = None,  # ⭐ 新增参数
        patch_size: int = 128,
        ...
        1）原来的用法（基于目录）：
            SegPatchDataset(image_dir="...", mask_dir="...", patch_size=128, ...)

        2）基于划分后的 pairs 列表：
            SegPatchDataset(pairs=[(img_path, mask_path), ...], patch_size=128, ...)
        """

        super().__init__()

        # ====== 1) 优先使用传入的 pairs（划分后的数据集） ======
        if pairs is not None:
            if not isinstance(pairs, list) or len(pairs) == 0:
                raise ValueError("`pairs` 不能为空，并且必须是 [(img_path, mask_path), ...] 的列表。")
            self.pairs = pairs

        # ====== 2) 否则退回到“目录 + 自动配对”的老用法 ======
        elif image_dir is not None and mask_dir is not None:
            self.pairs = pair_image_and_mask(image_dir, mask_dir)

        else:
            raise ValueError(
                "必须要么提供 `pairs`，要么同时提供 `image_dir` 和 `mask_dir`。"
            )
