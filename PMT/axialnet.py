...
class medt_net(nn.Module):
    ...
    def __init__(self, block, block_2, layers, num_classes=2, zero_init_residual=True,
        ...
        # block,block_2 类，被_make_layer反复调用，堆叠成layer1,layer2,layer3,layer4这些stage
        # layers 每个stage里的残差块个数
        # zero_init_residual 是否对残差分支的最后一个BN层做“零初始化”，常见技巧，为什么？默认为true
        # groups 分组卷积，每组独立卷积
        # width_per_group 每个group的“基准宽度”
        # replace_stride_with_dilation 是否用膨胀卷积代替下采样 默认为none 膨胀卷积和下采样的区别？
        # s 通道缩放系数，1/8，减小计算量
